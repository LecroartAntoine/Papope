...
export function CoachChat({ dateKey, currentItems, isClimbDay, onRefreshDay, onSupplementsUpdated }: Props) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/keeppushing/coach?date=${dateKey}`)
      .then(r => r.json())
      .then(data => {
  ...
  const send = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/keeppushing/coach', {
        method: 'POST',
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
   ...
   const suggestions = [
     "Cette semaine, je veux faire de l'escalade mardi et vendredi, 2 séances de Beat Saber et 2 séances de marche longue.",
     'Mes bras font très mal aujourd\'hui',
     "Génère une séance de force pour aujourd'hui",
    ...
    <>
      ...
      <button
       ...
       className="fixed bottom-6 right-6 z-50 max-w-14 max-h-14 overflow-hidden bg-black text-carbon rounded-full shadow-lg flex items-center justify-center hover:scale-105 transition-all active:scale-95"
         aria-label={t('keeppushing.denis')}
       >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
           {historyLoaded && messages.length === 0 && (
             <div className="text-center py-10">
               <div className="text-chalk text-sm font-mono font-bold mb-2">{t('keeppushing.yourTorturer')}</div>
            ...
            <div key={msg.id ?? idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[90%] flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                 <div className={`text-xs font-mono mb-1 ${msg.role === 'user' ? 'text-ash' : 'text-accent'}`}>
                   {msg.role === 'user' ? t('keeppushing.me') : t('keeppushing.denis')}
                 </div>
                <div className={`px-3 py-2.5 text-sm font-mono leading-relaxed whitespace-pre-wrap ${
                 ...
                 {msg.actions && (
                   <div className="mt-1.5 flex flex-wrap gap-1">
                     {msg.actions.weekPatch && <ActionChip icon="📅" label={t('keeppushing.weekPlanned')} color="#4EA8FF" />}
                     {msg.actions.dayPatch && <ActionChip icon="✏️" label={t('keeppushing.dayModified')} color="#F5A623" />}
                     {msg.actions.supplementPatch && <ActionChip icon="🥗" label={t('keeppushing.nutritionLogged')} color="#C8F135" />}
                     {msg.actions.supplementUpdates && <ActionChip icon="⚙️" label={t('keeppushing.supplementsUpdated')} color="#A78BFA" />}
                   </div>
                 )}
              </div>
            </div>
         ...
         {historyLoaded && messages.length === 0 && !loading && (
           <div className="px-4 pb-2 flex-shrink-0">
             <div className="text-xs text-ash font-mono mb-2 uppercase tracking-wider">{t('keeppushing.forExample')}</div>
        ...
        <div className="p-3 border-t border-steel flex-shrink-0 bg-slate">
          <div className="flex gap-2 items-end">
            <textarea
              ...
              placeholder={t('keeppushing.coachInputPlaceholder')}
              disabled={loading}
              rows={2}
           ...
           <div className="text-zinc text-xs font-mono mt-1.5 text-center">Denis</div>
        </div>
      </div>
    </>
  )
