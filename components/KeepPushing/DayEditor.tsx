...
function ItemEditor({ item, activityDef, onUpdate }: {
  ...
  return (
    <div className="space-y-3">
      <Field label={t('keeppushing.displayLabel')}>
        <TextInput value={item.label ?? ''} onChange={v => onUpdate({ label: v })} placeholder={item.activity} />
      </Field>
      {isStrength && (
        <div className="grid grid-cols-3 gap-2">
          <Field label={t('keeppushing.setsReps')}><TextInput value={item.sets ?? ''} onChange={v => onUpdate({ sets: v })} placeholder="3×8" /></Field>
          <Field label={t('keeppushing.weight')}><TextInput value={item.weight ?? ''} onChange={v => onUpdate({ weight: v })} placeholder="20kg" /></Field>
          <Field label={t('keeppushing.reps')}><TextInput value={item.reps ?? ''} onChange={v => onUpdate({ reps: v })} placeholder="8" /></Field>
        </div>
      )}
      {showDuration && (
        <Field label={t('keeppushing.duration')}>
      ...
      <Field label={t('keeppushing.notes')}>
...
function AddItemFlow({ activityDefs, onAdd, onCancel }: {
  ...
  if (step === 'category') {
    return (
      <div className="space-y-3">
        <div className="text-xs text-ash font-mono uppercase tracking-wider">{t('keeppushing.chooseCategory')}</div>
        <div className="grid grid-cols-2 gap-2">
        ...
        <button onClick={onCancel} className="text-xs text-ash font-mono hover:text-chalk">Cancel</button>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-3">
      ...
      <input autoFocus value={search} onChange={e => setSearch(e.target.value)}
        placeholder={t('keeppushing.searchOrCreate')}
        className="w-full bg-carbon border border-zinc text-chalk text-sm font-mono px-3 py-2 focus:outline-none focus:border-accent transition-colors" />
      <div className="max-h-48 overflow-y-auto space-y-1">
        ...
        {filtered.length === 0 && !search && (
          <div className="text-ash text-xs font-mono text-center py-4">Search to find or create</div>
        )}
      </div>
      {canCreate && (
        <div className="border border-accent border-opacity-40 bg-accent bg-opacity-5 p-3 space-y-2">
          <div className="text-accent text-xs font-mono">{t('keeppushing.create')} "{search.trim()}"</div>
          <div className="flex items-center gap-2">
...
export function DayEditor({
  ...
  useEffect(() => {
    if (open) {
      fetch('/api/keeppushing/activities').then(r => r.json()).then(data => {
        ...
        <div className="px-4 py-4 border-b border-steel flex items-center justify-between flex-shrink-0 bg-slate">
          <div className="flex items-center gap-3 flex-1 min-w-0">
           ...
           <div className="flex items-center gap-2 flex-shrink-0 ml-2">
             {hasPlan && (
               <button onClick={() => { if (confirm(t('keeppushing.clearThisDay'))) onClearDay() }}
