...
export function DailyWellbeing({ dateKey }: Props) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/keeppushing/wellbeing?date=${dateKey}`)
      .then(r => r.json())
      .then(data => {
        if (data && !data.error) {
          setState({
  ...
  useEffect(() => {
    ...
    setSaving(true)
    fetch('/api/keeppushing/wellbeing', {
      method: 'POST',
       ...
       <div className="px-5 py-4 border-b border-steel flex items-center justify-between">
         <div>
           <div className="flex items-center gap-2 mb-0.5">
             ...
             <span className="text-xs text-info tracking-[0.2em] uppercase font-mono">{t('keeppushing.wellbeing')}</span>
             {saving && <span className="text-zinc text-xs font-mono animate-pulse">{t('keeppushing.saving')}</span>}
           </div>
           <div className="font-display text-xl font-bold tracking-tight text-chalk">{t('keeppushing.dailyCheckIn')}</div>
         </div>
         <div className="text-right">
           <div className="font-mono font-bold text-lg" style={{ color: filled === total ? '#C8F135' : '#888' }}>
             {filled}/{total}
           </div>
           <div className="text-zinc text-xs">{t('keeppushing.filledIn')}</div>
         </div>
       </div>
      ...
      <div className="p-5 space-y-6">
         ...
         <div>
           ...
           <ScaleSelector
             ...
             lowLabel={t('keeppushing.veryBad')}
             highLabel={t('keeppushing.excellent')}
             colorFn={moodColor}
           />
         </div>
         ...
         <div>
           <div className="text-xs text-ash tracking-[0.2em] uppercase font-mono mb-3">
             {t('keeppushing.fatigue')}
           </div>
           <ScaleSelector
             value={state.fatigue}
             onChange={v => set('fatigue', v)}
             lowLabel={t('keeppushing.wellRested')}
             highLabel={t('keeppushing.exhausted')}
         ...
         <div>
           <div className="text-xs text-ash tracking-[0.2em] uppercase font-mono mb-3">
             {t('keeppushing.stress')}
           </div>
           <ScaleSelector
             value={state.stress}
             onChange={v => set('stress', v)}
             lowLabel={t('keeppushing.zen')}
             highLabel={t('keeppushing.veryStressed')}
             colorFn={badHighColor}
           />
         </div>
         ...
         <div>
           <div className="text-xs text-ash tracking-[0.2em] uppercase font-mono mb-3">
             {t('keeppushing.generalPain')}
           </div>
           <ScaleSelector
             value={state.douleur_generale}
             onChange={v => set('douleur_generale', v)}
             lowLabel={t('keeppushing.none')}
             highLabel={t('keeppushing.intense')}
             colorFn={painColor}
           />
         </div>
         ...
         <div>
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <span className="text-ash text-sm font-mono">{t('keeppushing.hours')}</span>
             {state.sommeil_h !== null && (
               <span className={`text-xs font-mono ml-2 ${
                 state.sommeil_h >= 7.5 ? 'text-accent' :
                 state.sommeil_h >= 6 ? 'text-warn' : 'text-crit'
               }`}>
                 {state.sommeil_h >= 7.5 ? t('keeppushing.optimal') : state.sommeil_h >= 6 ? t('keeppushing.insufficient') : t('keeppushing.tooShort')}
               </span>
             )}
          </div>
        </div>
         ...
         <div>
           <div className="text-xs text-ash tracking-[0.2em] uppercase font-mono mb-3">
             {t('keeppushing.alcoholConsumption')}
           </div>
          <div className="flex gap-2 flex-wrap">
            {[0, 1, 2, 3, 4, 5, 6, 7, 8].map(n => (
             ...
             <span className="text-zinc text-xs font-mono self-center ml-1">{t('keeppushing.glasses')}</span>
           </div>
         </div>
         ...
         <div>
           <div className="text-xs text-ash tracking-[0.2em] uppercase mb-2 font-mono">
             {t('keeppushing.personalNotes')}
           </div>
           <textarea
             value={state.notes_perso}
             onChange={e => set('notes_perso', e.target.value)}
             placeholder={t('keeppushing.howAreYouFeeling')}
