...
export function LiftLogger({ dateKey, dayPlan }: Props) {
  ...
  useEffect(() => {
    fetch(`/api/keeppushing/lifts?days=1`)
      .then(r => r.json())
      .then((data: any[]) => {
  ...
  const saveLog = async (exerciseId: string) => {
    ...
    try {
      await fetch('/api/keeppushing/lifts', {
        method: 'POST',
  ...
  if (exercises.length === 0) return null
   ...
   return (
     <div className="border-t border-steel pt-4">
       <div className="text-xs text-ash tracking-[0.2em] uppercase mb-3 font-mono">{t('keeppushing.logPerformance')}</div>
           ...
           return (
             <div key={ex.id} className="bg-carbon border border-zinc p-3 rounded">
               ...
               <div className="flex gap-2">
                 {[
                   { field: 'sets' as keyof LiftEntry, label: t('keeppushing.sets'), placeholder: '3' },
                   { field: 'reps' as keyof LiftEntry, label: t('keeppushing.reps'), placeholder: '8' },
