...
export function WeekOverview({ weekDays, onSelectDay }: Props) {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setPlans({})
    fetch(`/api/keeppushing/day-plan/week?from=${fromKey}&to=${toKey}`)
      .then(r => r.json())
      .then(data => {
        if (data && !data.error) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <div className="font-display text-5xl font-black tracking-tight text-chalk mb-1">{t('keeppushing.week')}</div>
                  ...
                  {plan ? (
                    <>
                      ...
                      <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                        ...
                        {total > 0 && (
                          <span className="text-xs font-mono text-ash">{done}/{total} {t('keeppushing.tasks')}</span>
                        )}
                      </div>
                    </>
                  ) : (
                    <div className="text-ash text-sm font-mono italic">
                      {loading ? t('common.loading') : t('keeppushing.emptyWeekDescription')}
                    </div>
                  )}
                </div>
