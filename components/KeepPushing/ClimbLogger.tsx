...
const STYLES = ['Flash', 'Redpoint', 'Moulinette', 'Attempt']
...
export function ClimbLogger({ dateKey }: { dateKey: string }) {
  ...
  useEffect(() => {
    fetch(`/api/keeppushing/climbs?date=${dateKey}`)
      .then(r => r.json())
  ...
  const addClimb = async () => {
    setAdding(true)
    try {
      const res = await fetch('/api/keeppushing/climbs', {
        method: 'POST',
  ...
  const removeClimb = async (id: number) => {
    await fetch(`/api/keeppushing/climbs?id=${id}`, { method: 'DELETE' }).catch(() => {})
   ...
   return (
     <div className="border-t border-steel pt-4">
       <div className="text-xs text-ash tracking-[0.2em] uppercase mb-3 font-mono">
         {t('keeppushing.routesCompleted', { count: climbs.length })}
       </div>
      ...
      {climbs.length > 0 && (
            ...
            <div key={c.id} className="flex items-center gap-2 bg-carbon border border-zinc px-3 py-2 rounded">
              ...
              <span className="text-ghost text-xs font-mono flex-1">{c.style}</span>
               <span className={`text-xs font-mono ${c.completed ? 'text-accent' : 'text-crit'}`}>
                 {c.completed ? t('keeppushing.sent') : t('keeppushing.project')}
               </span>
       ...
       <div className="bg-carbon border border-zinc p-3 rounded space-y-3">
         <div className="text-zinc text-xs font-mono">Add route</div>
         <div className="flex gap-2 flex-wrap">
           <div>
             <label className="text-zinc text-xs block mb-1">{t('keeppushing.grade')}</label>
           ...
           <div>
             <label className="text-zinc text-xs block mb-1">{t('keeppushing.style')}</label>
           ...
           <div className="flex items-end pb-0.5">
             <label className="flex items-center gap-2 cursor-pointer">
               ...
               <span className="text-xs text-ghost font-mono">{t('keeppushing.sent')}</span>
             </label>
           </div>
         </div>
         <div className="flex gap-2">
           <input type="text" placeholder={t('keeppushing.optionalNotes')} value={notes} onChange={e => setNotes(e.target.value)}
