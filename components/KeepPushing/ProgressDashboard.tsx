...
function WellbeingChart({ data }: { data: any[] }) {
  if (!data.length) return <Empty label="Fill in daily wellbeing to see curves" />
  return (
    <>
...
export function ProgressDashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`/api/keeppushing/wellbeing?days=${period}`).then(r => r.json()).catch(() => []),
      fetch(`/api/keeppushing/metrics?days=${period}`).then(r => r.json()).catch(() => []),
      fetch(`/api/keeppushing/supplement-logs?days=${period}`).then(r => r.json()).catch(() => []),
      fetch(`/api/keeppushing/supplements`).then(r => r.json()).catch(() => []),
      fetch(`/api/keeppushing/day-plan/week?from=${from}&to=${to}`).then(r => r.json()).catch(() => ({})),
    ...
    <div className="space-y-8">
      ...
      {!loading && !hasWellbeingData && !hasWeightData && !hasSuppData && !hasActivityData && (
        <div className="border border-dashed border-zinc p-12 text-center space-y-3">
          ...
          <div className="text-ash text-xs font-mono">Use the app for a few days pour voir tes stats apparaître ici.</div>
        </div>
      )}
    </div>
  )
