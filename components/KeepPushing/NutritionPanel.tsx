    ...
    <div className="border border-dashed border-accent border-opacity-40 bg-carbon p-4 space-y-3">
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="col-span-2">
          ...
          <input autoFocus value={name} onChange={e => setName(e.target.value)} placeholder="Ex: Magnésium"
            onKeyDown={e => e.key === 'Enter' && name.trim() && !saving && (async () => { setSaving(true); await fetch('/api/keeppushing/supplements',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'create',name:name.trim(),unit,target:target?parseFloat(target):null,emoji,hint:hint||null})}); onAdd(); onClose() })()}
            className="w-full bg-slate border border-zinc text-chalk text-sm font-mono px-3 py-2 focus:outline-none focus:border-accent" />
        </div>
        <div>
          <label className="block text-xs text-ash font-mono uppercase tracking-wider mb-1">Unité</label>
      ...
      <button
        onClick={async () => {
          if (!name.trim()) return
          setSaving(true)
          await fetch('/api/keeppushing/supplements', {
...
export function NutritionPanel({ dateKey, isClimbDay }: Props) {
  ...
  const loadSupplements = useCallback(async () => {
    const res = await fetch('/api/keeppushing/supplements')
  ...
  const loadLogs = useCallback(async () => {
    ...
    const res = await fetch(`/api/keeppushing/supplement-logs?date=${dateKey}`)
    const data = await res.json()
  ...
  useEffect(() => {
    ...
    setSaving(true)
    fetch('/api/keeppushing/supplement-logs', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
  ...
  const handleSaveSupp = async (id: number, updates: Partial<Supplement>) => {
    await fetch('/api/keeppushing/supplements', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'update', id, ...updates }) })
    await loadSupplements()
  }

  const handleDeleteSupp = async (id: number) => {
    await fetch('/api/keeppushing/supplements', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'delete', id }) })
    await loadSupplements()
  }
    ...
    <div className="space-y-4">
      <div className="border border-steel bg-slate">
         ...
         <div className="px-5 py-3 border-b border-steel flex items-center justify-between">
           <div className="text-xs text-ash tracking-[0.2em] uppercase font-mono">{t('keeppushing.todayNutrition')}</div>
           <div className="flex items-center gap-3">
             {saving && <span className="text-zinc text-xs font-mono animate-pulse">{t('keeppushing.saving')}</span>}
             <button onClick={() => { setManage(m => !m); setAddingNew(false); setEditingId(null) }}
               ...
               {manage ? t('keeppushing.done') : t('keeppushing.manage')}
             </button>
           </div>
         </div>
        ...
        <div className="p-4 space-y-5">
         ...
         {manage && (
           <div className="border-t border-steel px-4 pb-4 pt-4 space-y-2">
             <div className="text-xs text-ash font-mono uppercase tracking-wider mb-3">{t('keeppushing.allSupplements')}</div>
            {supplements.map(s => (
       ...
       <div className="border border-steel bg-slate p-4">
         <div className="text-xs text-ash tracking-[0.2em] uppercase font-mono mb-3">{t('keeppushing.bodyWeight')}</div>
