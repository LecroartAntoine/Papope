  ...
  return (
    <>
      <div className="space-y-8">
        ...
        <section>
          <div className="text-ash font-mono text-xs uppercase tracking-[0.2em] mb-3">🥗 {t('keeppushing.nutrition')}</div>
          <NutritionPanel key={nutritionKey} dateKey={dateKey} isClimbDay={isClimbDay} />
        </section>
        <section>
          <div className="text-ash font-mono text-xs uppercase tracking-[0.2em] mb-3">🌡 {t('keeppushing.wellbeingAndRecovery')}</div>
...
export function DashboardClient() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/keeppushing/day-plan/week?from=${from}&to=${to}`)
      .then(r => r.json())
      .then(data => {
        if (data && !data.error) {
  ...
  const navItems: { key: View; label: string; short: string }[] = [
    { key: 'jour',    label: "Today", short: 'Day'  },
    { key: 'semaine', label: 'Week',     short: 'Wk'  },
    { key: 'stats',   label: 'Progress',     short: 'Stats' },
        ...
        <div className="max-w-3xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button onClick={() => signOut({ callbackUrl: '/' })}
              className="text-ash hover:text-crit transition-colors p-1.5" title={t('keeppushing.logoutTooltip')}>
              <LogoutIcon />
            </button>
          </div>
        </div>
        ...
        {view === 'semaine' && <WeekOverview weekDays={weekDays} onSelectDay={d => { setSelectedDate(d); setView('jour') }} />}
         {view === 'stats'   && (
           <div className="space-y-6">
             <div>
               <div className="font-display text-5xl font-black tracking-tight text-chalk mb-1">{t('keeppushing.progress')}</div>
               <div className="text-ash font-mono text-sm">{t('keeppushing.yourDataOverTime')}</div>
             </div>
             <ProgressDashboard />
           </div>
         )}
